...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result) {
        ...
        $bill.html .= '<h3>Bill Details</h3><table class="table table-borderless"><thead><tr><th scope="col">Product ID</th><th scope="col">Product Name</th><th scope="col">Price</th><th scope="col">Quantity</th><th scope="col">Amount</th></tr></thead><tbody>';
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            $bill.html .= '<tr><td>' . $row['id'] . '</td><td>' . $row['title'] . '</td><td>' . $row['price'] . '</td><td>' . $row['qty'] . '</td><td>' . $order_amount . '</td></tr>';
        }

        $bill.html .= '</tbody></table>';
        $bill.html .= '<h3>Total Amount: ' . $total_amount . '</h3>';
    } else {
